...
> Build: `dirty candidates build 2026-05-15 r10`
...
- **스텝 단위 시각화**: BSP 분할 → 방 배치 → 방 채움 → MST 통로 → **소스룸 단위 EXTRA 통로(체크 → pair 스킵 → 후보군 → pair-best 모음 → carve)** → 계단 배치 전 과정을 스냅샷으로 저장하고, 화살표 / 슬라이더 / Play 버튼으로 스크럽 가능.
- **EXTRA 다중 후보 오버레이**: 한 스텝에서 여러 후보 경로를 **16색 팔레트로 동시 표시**하고, 그 중 선택된(best) 경로를 빨간 두꺼운 테두리로 강조.
- **DIRTY 후보 오버레이**: 거절 규칙에 걸린 더티 후보를 회색(`#777777`)으로 표시 — `Dirty` 토글로 ON/OFF 가능.
...
| 좌클릭 드래그 | 캔버스 이동(패닝) |
| `BSP` / `Rooms` / `Path` / `Dirty` 토글 | 오버레이 표시 토글 (`Path` 토글은 EXTRA 후보군 오버레이도 함께 제어, `Dirty` 토글은 거절된 더티 후보 표시 ON/OFF) |
...
│   ├─ Canvas (PIL 렌더링 + 캐시 + 다색 EXTRA/DIRTY 오버레이) │
...
[DungeonGenerate_Simulator.py:185](DungeonGenerate_Simulator.py#L185)
...
[`DungeonGenerator.generate()`](DungeonGenerate_Simulator.py#L310) 이 단계마다 `snapshot()` 을 호출해 `Step` 리스트를 누적합니다.
...
| 05 | `connect_extra_corridors` | **EXTRA**: 소스룸 사이클을 돌며 체크 → pair 스킵 → 후보군 → pair-best 모음 → carve 의 스텝을 생성 |
| 06 | `place_stairs` | 셔플된 방 순서로 적합한 셀을 찾아 `STAIR_UP` 배치 (`floor < max_floor` 조건) |
| 99 | 최종 | MST + EXTRA + 계단이 모두 적용된 최종 던전 스냅샷 |

#### MST 단계

[`connect_all`](DungeonGenerate_Simulator.py#L386)
...
#### EXTRA 통로 단계 — **소스룸 사이클 방식**

[`connect_extra_corridors`](DungeonGenerate_Simulator.py#L431)

MST 완료 후 방 인덱스 순서(0..n-1)로 한 방씩 순회하며 그 방을 **소스룸**으로 삼아 EXTRA 통로를 한 번 만들 기회를 부여합니다. 스텝 시각화도 이 흐름에 맞춰 세분화됩니다.

소스룸 1회 사이클의 스텝 구성:

| 스텝 타이틀 | 내용 |
|---|---|
| `05-R{XX}. EXTRA 생성 체크: 진행` | `ExtraConnProb` 롤 통과 — 이 소스룸은 EXTRA를 시도함 |
| `05-R{XX}. EXTRA 생성 체크: 스킵` | 롤 실패 — 이 소스룸은 EXTRA를 만들지 않고 다음 룸으로 |
| `05-R{XX}. pair R{src} -> R{dst} 스킵` | 해당 pair가 MST 또는 이전 EXTRA로 이미 연결된 경우 |
| `05-R{XX}. R{src} -> R{dst} 후보 없음` | 그 쌍에서 거절 규칙을 통과한 후보가 없을 때, dirty 사유 목록과 함께 표시 |
| `05-R{XX}. R{src} -> R{dst} 후보군 + pair best` | **그 쌍의 모든 클린 후보를 16색 팔레트로, dirty 후보를 회색으로 동시 표시**, pair-best는 빨간 두꺼운 테두리 |
| `05-R{XX}. source R{src} pair-best 후보군 + 최종 best` | **그 소스룸의 모든 pair-best 경로를 한 화면에 모아 표시**, 그중 source-best 를 빨간 강조 |
| `05-R{XX}. source R{src} EXTRA 통로 생성 R{src} -> R{dst}` | source-best 를 carve. 격자에 `CORRIDOR` 가 실제로 새겨지는 순간 |
| `05-R{XX}. source R{src} EXTRA 선택 없음` | 모든 쌍에서 후보가 비었을 때(드물지만 가능) |
...
3. 한 쌍당 `ExtraCandidateCount` 변형 경로 생성([`emit_extra_path_candidate`](DungeonGenerate_Simulator.py#L675), [`pick_extra_axis`](DungeonGenerate_Simulator.py#L693)).
4. 각 후보는 **8개 거절 규칙**([`corridor_candidate_reject_reason`](DungeonGenerate_Simulator.py#L970))으로 필터링:
...
5. 거절된 후보는 **dirty 후보**로 회색 오버레이에 표시 (`Dirty` 토글 ON 시). 살아남은 후보는 [`score_extra_corridor_candidate`](DungeonGenerate_Simulator.py#L756) 로 점수화 — **가중치가 UI에서 노출**됨:
   ```
...
6. 쌍별 최선([`select_best_extra_corridor_candidate`](DungeonGenerate_Simulator.py#L645)) → 소스룸 전체에서 다시 최선 → carve. 동률 시 [`is_better_extra_corridor_candidate`](DungeonGenerate_Simulator.py#L653) 의 결정론적 tie-break(score / path_len / overlap / dist / src / dst / candidate / attempt / primary) 적용.
7. carve 한 쌍은 `connected_pairs` 에 추가되어 이후 소스룸에서 중복 선택되지 않음. EXTRA는 MST의 `connected/remaining` 상태를 변경하지 않음.

#### EXTRA 후보 오버레이

[`build_extra_candidate_overlays`](DungeonGenerate_Simulator.py#L283)

- 한 스텝에 여러 후보 경로를 함께 표시하기 위한 `(path, color, label)` 리스트를 만듦.
- 16색 고대비 팔레트 ([`extra_candidate_color`](DungeonGenerate_Simulator.py#L272)) 가 후보별로 순차 배정 — `#00e5ff`, `#ffd60a`, `#7cff6b`, `#ff7ad9`, ...
- 선택된 best 후보는 팔레트가 아닌 빨간색 (`#ff3b30`) 으로 별도 표시.
- 거절된 dirty 후보는 회색(`#777777`)으로 표시되며 라벨이 `DIRTY C{n}:` 으로 시작함.
- 각 후보 경로의 중간 셀에 `C0 {score}`, `C1 {score}`, … `S {score}`(선택됨), `D{n} {score}`(dirty) 라벨이 작은 텍스트로 그려짐.

### 3.4 데이터 모델

[DungeonGenerate_Simulator.py:143-243](DungeonGenerate_Simulator.py#L143-L243)
...
| `Step` | `title`, 격자 복사본, 방/노드 스냅샷, 강조 경로, 디버그 노트, **`extra_paths`(다색 후보 오버레이 리스트, dirty 포함)** |

### 3.5 UI 계층

[`DungeonViewer`](DungeonGenerate_Simulator.py#L1286)

- **렌더링**: `Step.grid` → PIL `Image.putdata` (1픽셀 = 1셀) → `Image.resize(NEAREST)` 로 확대 → `ImageDraw` 로 BSP / Room / **EXTRA 다색 후보 / DIRTY 회색 후보** / 선택 경로 오버레이 → `ImageTk.PhotoImage`.
- **EXTRA 후보 그리기**: 클린 후보 경로는 셀 내부 1px 안쪽 테두리(`cell//3` 두께), dirty 후보는 동일 두께 회색 테두리, 선택 경로는 셀 외곽 두꺼운 테두리(`cell//2` 두께)로 그려 시각적 우선순위 부여.
- **캐시**: `(step_index, cell, show_bsp, show_rooms, show_path, show_dirty)` 키로 PhotoImage 캐시. LRU 96개 제한.
...
- **범례**: `EMPTY` / `ROOM` / `CORRIDOR` / `STAIR_UP` / `selected path` / **`EXTRA candidates`** / **`DIRTY candidates`** 7종 표시.
...
DIRTY candidates     #777777  (거절된 후보)
Room outline         #e8f1a1
BSP outline          #3b4a66
```
